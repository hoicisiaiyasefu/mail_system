...
"""
垃圾邮件检测桥接脚本
通过命令行参数接收 JSON 邮件数据，调用 MLSpamDetector 处理

用法:
  python bridge_spam.py '<json_email_data>'                               # 无配置
  python bridge_spam.py '<json_email_data>' '<json_config>'               # 有配置
"""
...
def main():
    ...
    if len(sys.argv) >= 3:
        try:
            ...
            if arg2.endswith('.json'):
                config_dict = {"plugins": {"spam_ml_plugin": {"enabled": True}}, "llm": {"enabled": False}}
            else:
    ...
    result = registry.process("spam_ml_plugin", email_data)
